...
var errorId = require('chalk').gray;
...
var display = function display(reports) {
  reports.forEach(function (r) {
    ...
    r.reports.forEach(function (r) {
      console.log("\t".concat(r.line, ":").concat(r.col + 1, "\t").concat(r.severity == 'ERROR' ? error("Error\t") : "").concat(r.severity == 'WARNING' ? warning("Warning\t") : "", "\t").concat(r.message, "\t").concat(errorId(r.rule_name)));
    });
  });
};
