...
import { Token, descKeywordTable } from '../token.mjs';
import { Chars } from '../chars.mjs';
import { advanceChar, consumeMultiUnitCodePoint, fromCodePoint, toHex } from './common.mjs';
import { CharTypes, CharFlags, isIdentifierPart, isIdentifierStart, isIdPart } from './charClassifier.mjs';
import { report, reportScannerError, Errors } from '../errors.mjs';
 ...
 * @param context Context masks
 */
export function scanMetaVariable(parser: ParserState, context: Context, isValidAsKeyword: 0 | 1): Token {
  while (isIdPart[advanceChar(parser)]) {}
  parser.tokenValue = parser.source.slice(parser.tokenPos, parser.index);
 ...
 */
export function scanMetaVariableSlowCase(
  parser: ParserState,
  context: Context,
  hasEscape: 0 | 1,
  ...
  if (isValidAsKeyword && length >= 2 && length <= 11) {
    ...
    if (token === void 0) return Token.MetaVariable;
