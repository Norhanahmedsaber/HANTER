...
var _index = require("./lexer/index.js");
var _token6 = require("./token.js");
var _errors = require("./errors.js");
var _template = require("./lexer/template.js");
var _jsx = require("./lexer/jsx.js");
...
function parseSource(source, options, context) {
  ...
  if (context & 1) (0, _index.skipHashBang)(parser);
...
function parseStatementList(parser, context, scope) {
  (0, _index.nextToken)(parser, context | 32768 | 1073741824);
  var statements = [];
...
function parseModuleItemList(parser, context, scope) {
  (0, _index.nextToken)(parser, context | 32768);
  var statements = [];
  if (context & 8) {
    while (parser.token === 134283267) {
...
function parseStatementListItem(parser, context, scope, origin, labels) {
  ...
  switch (parser.token) {
    ...
    case 86108:
      (0, _index.nextToken)(parser, context);
      switch (parser.token) {
        case 67174411:
...
function parseMemberOrUpdateExpression(parser, context, expr, inGroup, inChain, start, line, column) {
  if ((parser.token & 33619968) === 33619968 && (parser.flags & 1) === 0) {
    ...
    switch (parser.token) {
      case 67108877:
        {
          (0, _index.nextToken)(parser, (context | 1073741824 | 8192) ^ 8192);
          parser.assignable = 1;
...
function parseIdentifier(parser, context, isPattern) {
  var tokenValue = parser.tokenValue,
    ...
    colPos = parser.colPos;
  (0, _index.nextToken)(parser, context);
  return tokenValue[0] == '$' ? (0, _common.finishNode)(parser, context, tokenPos, linePos, colPos, context & 268435456 ? {
    type: 'MetaVariable',
    ...
    type: 'MetaVariable',
    name: tokenValue.slice(1)
  }) : (0, _common.finishNode)(parser, context, tokenPos, linePos, colPos, context & 268435456 ? {
    type: 'Identifier',
    name: tokenValue,
    pattern: isPattern === 1
  } : {
...
function parseObjectLiteralOrPattern(parser, context, scope, skipInitializer, inGroup, isPattern, kind, origin, start, line, column) {
  ...
  while (parser.token !== 1074790415) {
    ...
    if (token === 14) {
      ...
      var value = void 0;
      var t = parser.token;
      if (parser.token & (143360 | 4096 | 139) || parser.token === 121) {
        if (parser.token == 139) {
...
function parseMembeExpressionNoCall(parser, context, expr, inGroup, start, line, column) {
  ...
  if (token & 67108864) {
    if (token === 67108877) {
      (0, _index.nextToken)(parser, context | 1073741824);
      parser.assignable = 1;
      var property = parsePropertyOrPrivatePropertyName(parser, context);
...
function parseBindingPattern(parser, context, scope, type, origin, start, line, column) {
  if (parser.token & 143360) return parseAndClassifyIdentifier(parser, context, scope, type, origin, start, line, column);else if (parser.token == 139) {
    return parseGeneral(parser, context, parser.tokenPos, parser.linePos, parser.colPos);
  }
...
function parseAndClassifyIdentifier(parser, context, scope, kind, origin, start, line, column) {
  ...
  (0, _index.nextToken)(parser, context);
  if (scope) (0, _common.addVarOrBlock)(parser, context, scope, tokenValue, kind, origin);
  return tokenValue[0] == '$' ? (0, _common.finishNode)(parser, context, start, line, column, {
    type: 'MetaVariable',
    name: tokenValue.slice(1)
  }) : (0, _common.finishNode)(parser, context, start, line, column, {
    type: 'Identifier',
