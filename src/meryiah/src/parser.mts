import { nextToken, skipHashBang } from './lexer/index.mjs';
import { Token, KeywordDescTable } from './token.mjs';
import * as ESTree from './estree.mjs';
import { report, reportMessageAt, reportScopeError, Errors } from './errors.mjs';
import { scanTemplateTail } from './lexer/template.mjs';
import { scanJSXIdentifier, scanJSXToken, scanJSXAttributeValue } from './lexer/jsx.mjs';
import {
  Context,
  ParserState,
  PropertyKind,
...
export function parseModuleItem(parser: ParserState, context: Context, scope: ScopeState | undefined): any {
  ...
  switch (parser.token) {
    case Token.General:
      moduleItem = parseGeneral(parser, context, parser.tokenPos, parser.linePos, parser.colPos);
      break;
...
export function parseLabelledStatement(
  ...
  return finishNode(parser, context, start, line, column, {
    ...
    label: expr as ESTree.Identifier | ESTree.MetaVariable,
    body
  });
}
...
export function parseContinueStatement(
  ...
  nextToken(parser, context);
  let label: ESTree.Identifier | ESTree.MetaVariable | undefined | null = null;
...
export function parseRestrictedIdentifier(
  ...
  context: Context,
  scope: ScopeState | undefined
): ESTree.Identifier | ESTree.MetaVariable {
...
function parseImportSpecifierOrNamedImports(
  ...
  while (parser.token & Token.IsIdentifier) {
    ...
    let local: ESTree.Identifier | ESTree.MetaVariable;
...
function parseExportDeclaration(
  ...
  switch (parser.token) {
    case Token.Multiply: {
      ...
      let exported: ESTree.Identifier | ESTree.MetaVariable | null = null;
    ...
    case Token.LeftBrace: {
      ...
      while (parser.token & Token.IsIdentifier) {
        ...
        let exported: ESTree.Identifier | ESTree.MetaVariable | null;
...
export function parseAsyncExpression(
  ...
  line: number,
  column: number
): ESTree.FunctionExpression | ESTree.ArrowFunctionExpression | ESTree.CallExpression | ESTree.Identifier | ESTree.MetaVariable {
  const { token } = parser;
...
export function parseYieldExpression(
  ...
  line: number,
  column: number
): ESTree.YieldExpression | ESTree.Identifier | ESTree.MetaVariable | ESTree.ArrowFunctionExpression {
  // YieldExpression[In] :
  //     yield
...
function parseImportCallOrMetaExpression(
  ...
  let expr: ESTree.Identifier | ESTree.MetaVariable | ESTree.ImportExpression = parseIdentifier(parser, context, 0);
...
export function parseImportMetaExpression(
  parser: ParserState,
  context: Context,
  meta: ESTree.Identifier | ESTree.MetaVariable,
  start: number,
  line: number,
  column: number
): ESTree.MetaProperty {
...
export function parseArguments(
  ...
  while (parser.token !== Token.RightParen) {
    if (parser.token === Token.Ellipsis) {
      ...
      args.push(parseGeneral(parser, context, parser.tokenPos, parser.linePos, parser.colPos))
    } else {
 ...
 * @param context Context masks
 */
export function parseIdentifier(parser: ParserState, context: Context, isPattern: 0 | 1): ESTree.Identifier | ESTree.MetaVariable {
...
export function parseFunctionDeclaration(
  ...
  let id: ESTree.Identifier | ESTree.MetaVariable | null = null;
...
export function parseArrayExpressionOrPattern(
  ...
  const elements: (ESTree.Identifier | ESTree.MetaVariable | ESTree.AssignmentExpression | null)[] = [];
  let destructible: AssignmentKind | DestructuringKind = 0;
  ...
  while (parser.token !== Token.Comma) {
    ...
    if (parser.token === Token.Assign) {
      ...
      left = finishNode(parser, context, tokenPos, linePos, colPos, {
        type: 'AssignmentPattern',
        left: left as ESTree.BindingPattern | ESTree.Identifier | ESTree.MetaVariable,
        right
      });
    }
...
export function parseIdentifierOrArrow(
  ...
  line: number,
  column: number
): ESTree.Identifier | ESTree.MetaVariable | ESTree.ArrowFunctionExpression {
  const { tokenValue } = parser;
...
export function parseAsyncArrowOrCallExpression(
  parser: ParserState,
  context: Context,
  callee: ESTree.Identifier | ESTree.MetaVariable | void,
  canAssign: 0 | 1,
  kind: BindingKind,
  origin: Origin,
  flags: Flags,
