...
function matchVariableDeclarator(targetednNode, node) {
  ...
  // id Checking
  switch (_variableDeclaratorId["default"][targetednNode.id.type]) {
    case 'Identifier':
...
function matchProperty(targetednNode, node) {
  ...
  switch (_property["default"][targetednNode.value.type]) {
    case "Identifier":
...
function matchObjectPattern(targetedNode, node) {
  var _iterator3 = _createForOfIteratorHelper(targetedNode.properties),
    _step3;
  try {
    for (_iterator3.s(); !(_step3 = _iterator3.n()).done;) {
      var targetedProperty = _step3.value;
      var found = false;
      var _iterator4 = _createForOfIteratorHelper(node.properties),
        _step4;
      try {
        for (_iterator4.s(); !(_step4 = _iterator4.n()).done;) {
          var nodeProperty = _step4.value;
