...
exports.scanMetaVariable = scanMetaVariable;
exports.scanMetaVariableSlowCase = scanMetaVariableSlowCase;
...
function scanMetaVariable(parser, context, isValidAsKeyword) {
  while (_charClassifier.isIdPart[(0, _common.advanceChar)(parser)]) {}
  parser.tokenValue = parser.source.slice(parser.tokenPos, parser.index);
  return parser.currentChar !== 92 && parser.currentChar <= 0x7e ? 143500 : scanIdentifierSlowCase(parser, context, 0, isValidAsKeyword);
}
...
function scanMetaVariableSlowCase(parser, context, hasEscape, isValidAsKeyword) {
