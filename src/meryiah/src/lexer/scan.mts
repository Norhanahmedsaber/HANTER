...
import { ParserState, Context, Flags } from '../common.mjs';
import { report, Errors } from '../errors.mjs';
import { unicodeLookup } from '../unicode.mjs';
import {
  advanceChar,
  LexerState,
...
import { skipSingleLineComment, skipMultiLineComment, skipSingleHTMLComment, CommentType } from './comments.mjs';
import { scanRegularExpression } from './regexp.mjs';
import { scanTemplate } from './template.mjs';
import { scanNumber } from './numeric.mjs';
import { scanString } from './string.mjs';
import { scanIdentifier, scanUnicodeIdentifier, scanIdentifierSlowCase, scanPrivateIdentifier, scanMetaVariable } from './identifier.mjs';
      ...
      switch (token) {
        ...
        case Token.Period:
          ...
          if (next === Chars.Period) {
            const index = parser.index + 1;
            if (index < parser.end && source.charCodeAt(index) === Chars.Period && source.charCodeAt(index+1) === Chars.Period && source.charCodeAt(index+2) === Chars.Period) {
              parser.column += 4;
        ...
        // $
        case Token.MetaVariable:
          advanceChar(parser);
          return scanMetaVariable(parser, context, /* isValidAsKeyword */ 0);
